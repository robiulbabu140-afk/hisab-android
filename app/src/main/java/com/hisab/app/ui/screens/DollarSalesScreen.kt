...
@Composable
fun DollarSalesScreen(navController: NavController) {
    val container = LocalAppContainer.current
    val scope = rememberCoroutineScope()
    var buyers by remember { mutableStateOf<List<BuyerDue>?>(null) }
    ...
    Column(Modifier.fillMaxSize()) {
        ScreenHeader("Dollar Sale Buyers", onBack = { navController.popBackStack() })
        when {
            loadError != null -> EmptyState("⚠️", "লোড করা যায়নি", loadError ?: "")
            buyers == null -> Column(Modifier.fillMaxSize().padding(32.dp)) { CircularProgressIndicator() }
            buyers!!.isEmpty() -> EmptyState("💵", "কোনো buyer নেই", "Web dashboard-এর 💵 Dollar পেজ থেকে Sell করুন।")
            else -> LazyColumn(Modifier.fillMaxSize().padding(horizontal = 18.dp)) {
                items(buyers!!, key = { it.buyerName }) { buyer ->
                    BuyerRow(buyer) { reload() }
                }
            }
        }
    }
...
@Composable
private fun BuyerRow(buyer: BuyerDue, onPaid: () -> Unit) {
    val container = LocalAppContainer.current
    val scope = rememberCoroutineScope()
    val accounts by container.accountRepository.observeAll().collectAsState(initial = emptyList())

    var expanded by remember(buyer.buyerName) { mutableStateOf(false) }
    var amount by remember(buyer.buyerName) { mutableStateOf("") }
    var accountId by remember(buyer.buyerName) { mutableStateOf<Long?>(null) }
    var error by remember(buyer.buyerName) { mutableStateOf<String?>(null) }
    var submitting by remember(buyer.buyerName) { mutableStateOf(false) }

    HisabCard(modifier = Modifier.padding(bottom = 12.dp).clickable { expanded = !expanded }) {
        Row(Modifier.fillMaxWidth()) {
            Column(Modifier.weight(1f)) {
                Text(buyer.buyerName, style = MaterialTheme.typography.titleMedium)
                Text("${"%.2f".format(buyer.dollarCents / 100.0)} বিক্রি", style = MaterialTheme.typography.bodySmall)
            }
            Text(
                if (buyer.dueMinor > 0) "Due ${Money.format(buyer.dueMinor)}" else "Settled",
                color = if (buyer.dueMinor > 0) HisabRed else MaterialTheme.colorScheme.primary,
